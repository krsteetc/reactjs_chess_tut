...
function Board(props) {
  ...
  function movePiece(x, y) {
    ...
    const removeFrom = props.squares.findIndex(
      (obj) => obj.x === selectedPiece.x && obj.y === selectedPiece.y
    );

    const updatedSquares = [...props.squares];
    updatedSquares[moveTo] = {
      ...updatedSquares[moveTo],
      type: selectedPiece.type,
      isEmpty: false,
    };
    updatedSquares[removeFrom] = {
      ...updatedSquares[removeFrom],
      type: "empty",
      isEmpty: true,
    };
    ...
    switch (figure) {
      case "pawn":
        if (color === "white") {
          if (piece.y === 2) {
            let endFlag = true;
            for (let i = 1; i <= 2; i++) {
              const setLegal = props.squares.findIndex(
                (obj) => obj.x === piece.x && obj.y === i + piece.y
              );
              if (props.squares[setLegal].isEmpty) {
                updatedLegalMoves[setLegal] = {
                  ...updatedLegalMoves[setLegal],
                  isLegal: true,
                };
                endFlag = false;
          ...
          const attackLeft = props.squares.findIndex(
            (obj) =>
              obj.x === piece.x - 1 && obj.y === piece.y + 1 && !obj.isEmpty
          );
          const attackRight = props.squares.findIndex(
            (obj) =>
              obj.x === piece.x + 1 && obj.y === piece.y + 1 && !obj.isEmpty
          );
          ...
          if (
            attackLeft !== -1 &&
            props.squares[attackLeft].type.split("_")[1] !== "white"
          ) {
            updatedLegalMoves[attackLeft] = {
              ...updatedLegalMoves[attackLeft],
              isLegal: true,
            };
          }
          if (
            attackRight !== -1 &&
            props.squares[attackRight].type.split("_")[1] !== "white"
          ) {
            updatedLegalMoves[attackRight] = {
              ...updatedLegalMoves[attackRight],
              isLegal: true,
            };
          }
          ...
          if (piece.y === 7) {
            ...
            for (let i = 1; i <= 2; i++) {
              const setLegal = props.squares.findIndex(
                (obj) => obj.x === piece.x && obj.y === piece.y - i
              );
              if (props.squares[setLegal].isEmpty) {
                updatedLegalMoves[setLegal] = {
                  ...updatedLegalMoves[setLegal],
          ...
          if (
            attackLeft !== -1 &&
            props.squares[attackLeft].type.split("_")[1] !== "black"
          ) {
            updatedLegalMoves[attackLeft] = {
              ...updatedLegalMoves[attackLeft],
              isLegal: true,
          ...
          if (
            attackRight !== -1 &&
            props.squares[attackRight].type.split("_")[1] !== "black"
          ) {
            updatedLegalMoves[attackRight] = {
              ...updatedLegalMoves[attackRight],
              isLegal: true,
      ...
      case "rook":
        ...
        for (let i = 1; i < maxUp; i++) {
          let endFlag = true;
          const up = props.squares.findIndex(
            (obj) => obj.x === piece.x && obj.y === piece.y + i
          );
        ...
        for (let i = 1; i < maxDown; i++) {
          let endFlag = true;
          const down = props.squares.findIndex(
            (obj) => obj.x === piece.x && obj.y === piece.y - i
          );
          if (props.squares[down].isEmpty) {
            updatedLegalMoves[down] = {
              ...updatedLegalMoves[down],
              isLegal: true,
            };
            endFlag = false;
        ...
        for (let i = 1; i < maxRight; i++) {
          let endFlag = true;
          const right = props.squares.findIndex(
            (obj) => obj.x === piece.x + i && obj.y === piece.y
          );
          if (props.squares[right].isEmpty) {
            updatedLegalMoves[right] = {
              ...updatedLegalMoves[right],
              isLegal: true,
            };
            endFlag = false;
        ...
        for (let i = 1; i < maxLeft; i++) {
          let endFlag = true;
          const left = props.squares.findIndex(
            (obj) => obj.x === piece.x - i && obj.y === piece.y
          );
          if (props.squares[left].isEmpty) {
            updatedLegalMoves[left] = {
              ...updatedLegalMoves[left],
              isLegal: true,
            };
            endFlag = false;
      ...
      case "bishop":
        ...
        for (let dir of directions) {
          ...
          while (newX >= 1 && newX <= 8 && newY >= 1 && newY <= 8) {
            const index = props.squares.findIndex(
              (obj) => obj.x === newX && obj.y === newY
            );
            const square = props.squares[index];

            if (square.isEmpty) {
              updatedLegalMoves[index] = {
                ...updatedLegalMoves[index],
                isLegal: true,
              };
